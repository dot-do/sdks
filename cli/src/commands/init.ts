 ...
 */
function print(message: string, color?: 'green' | 'yellow' | 'red' | 'cyan' | 'dim') {
...
async function createGitHubRepo(
  name: string,
  targetDir: string,
  options: { org?: string; private?: boolean }
): Promise<boolean> {
  ...
  try {
    ...
    execSync(
      `gh repo create ${org}/${repoName} ${visibility} --source=. --remote=origin`,
