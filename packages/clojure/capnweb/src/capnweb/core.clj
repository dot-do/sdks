(ns capnweb.core
  "Cap'n Web Clojure Client - Stub SDK for Conformance Testing.
...
(deftype RpcStub [session path]
  ...
  (-call [_ method args]
    (let [request-id (-next-id session)
      ...
      ;; Return a new stub that wraps the deferred for chaining
  ...
  clojure.lang.IDeref
  (deref [this]
    ;; Dereferencing an unexecuted stub triggers execution
    (let [request-id (-next-id session)
          message {:id request-id
                   :type "call"
                   :pipeline (mapv (fn [step]
...
(deftype DeferredStub [session path result-deferred]
  ...
  (-call [_ method args]
    ;; This would require re-executing with extended pipeline
    ;; For stub purposes, we chain on the deferred
    (let [new-path (conj path {:op :call :method method :args (vec args)})
          ;; In real implementation, this would send a new pipelined request
          ;; For stub, we simulate by chaining
          chained-d (d/chain result-deferred
                             (fn [_result]
                               ;; Real impl would call method on result
...
(defn connect
  ...
  ([url options]
   (let [pending-requests (atom {})
         ...
         ;; Create session first (ws-stream will be set after connection)
         ...
         connect-d (d/timeout!
                    (d/catch
                     ...
                     (fn [e]
                       ;; If WebSocket fails, return nil (offline mode for testing)
                       nil))
                    (or (:timeout-ms options) 5000)
                    nil)]

     ;; Set up message handling when connection succeeds
     (d/on-realized connect-d
                    (fn [ws]
                      (when ws
                        ;; Update session with actual stream
                        (alter-var-root #'capnweb.core/dummy-var (constantly nil))
     ...
     ;; Return session with the ws-stream
     (let [ws @connect-d]
       (assoc session :ws-stream ws)))))

;; Dummy var for alter-var-root (needed for session update)
(def ^:private dummy-var nil)
...
(defn call
  "Invoke a method on an RPC stub or capability.
   ...
   Usage:
     (rpc/call api :method arg1 arg2)
     (-> api (rpc/call :method arg1 arg2))
     (-> api :property (rpc/call :method arg1))"
  [stub method & args]
  (if (instance? RpcStub stub)
    (-call stub method args)
    (if (instance? DeferredStub stub)
      (-call stub method args)
      ;; Allow calling on maps for testing
      (if (map? stub)
        (d/success-deferred
  ...
  "Server-side map operation over a remote collection.
   ...
   Usage:
     (-> api
         ...
         (rpc/rmap (fn [x] (-> api (rpc/call :square x)))))

   The function receives each element and should return
   an RPC pipeline that will be executed server-side.

   For conformance testing, this creates a pipeline step
   that the server interprets as a map operation."
  [coll-stub f]
  (cond
    ;; For RpcStub, add rmap to the pipeline
    (instance? RpcStub coll-stub)
    (let [session (-session coll-stub)
          path (-path coll-stub)
          ;; Create a synthetic element to extract the pipeline
          ;; In real impl, we'd serialize the function
          new-path (conj path {:op :rmap
                               :captures ["$self"]
                               :fn-pipeline []})]
      (->RpcStub session new-path))

    ;; For DeferredStub, chain rmap on the result
          ...
          mapped-d (d/chain result-d
                            (fn [coll]
                              (cond
                                ...
                                ;; In real impl, this happens server-side
                                (mapv (fn [x]
                                        (let [result (f x)]
...
(defn try-deref
  "Safely dereference a pipeline, returning [value nil] or [nil error].
   ...
   Usage:
     (let [[result err] (rpc/try-deref pipeline)]
       (if err
         (handle-error err)
         (use-result result)))"
  [pipeline]
