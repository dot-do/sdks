      ...
      this.pendingRequests.set(id, {
        resolve: (response) => {
          ...
          if (rpcResp.error) {
            ...
            resolve2(this.processResult(rpcResp.result));
          }
        },
        reject
      });
  ...
  handleMessage(data) {
    try {
      ...
      if ("id" in message && this.pendingRequests.has(message.id)) {
        const { resolve: resolve2 } = this.pendingRequests.get(message.id);
...
// src/cli.ts
var __dirname = path2.dirname((0, import_url.fileURLToPath)(importMetaUrl));
...
program.name("conformance-runner").description("Cross-language conformance test runner for DotDo RPC implementations").version(version);
program.option(
  "-s, --server-url <url>",
...
function findDefaultTestPath() {
  const possiblePaths = [
    path2.resolve(__dirname, "../../../../test/conformance"),
    path2.resolve(__dirname, "../../../test/conformance"),
    path2.resolve(process.cwd(), "test/conformance"),
    path2.resolve(process.cwd(), "conformance")
  ];
...
function formatResults(result, format, verbose) {
  switch (format) {
    case "json":
...
async function main() {
  try {
    ...
    if (!validFormats.includes(opts.output)) {
      console.error(import_picocolors2.default.red(`Invalid output format: ${opts.output}`));
      console.error(`Valid formats: ${validFormats.join(", ")}`);
      process.exit(1);
    }
    console.error(import_picocolors2.default.dim(`Loading tests from: ${opts.testPath}`));
    let specs = loadTestSpecs(opts.testPath);
    if (opts.suite && opts.suite.length > 0) {
      specs = filterBySuites(specs, opts.suite);
      if (specs.length === 0) {
        console.error(import_picocolors2.default.red("No matching suites found"));
        process.exit(1);
      }
    }
    if (opts.filter) {
      ...
      if (specs.length === 0) {
        console.error(import_picocolors2.default.red("No tests match the filter pattern"));
        process.exit(1);
      }
    }
    if (opts.list) {
      const summary2 = getSpecsSummary(specs);
      ...
      for (const spec of specs) {
        console.log(`
${import_picocolors2.default.bold(spec.name)}`);
        for (const test of spec.tests) {
          console.log(`  - ${test.name}`);
          if (test.description && opts.verbose) {
            console.log(import_picocolors2.default.dim(`    ${test.description}`));
          }
        }
      }
      console.log("\n" + import_picocolors2.default.dim("\u2500".repeat(50)));
      console.log(
        `
Total: ${summary2.totalSuites} suites, ${summary2.totalTests} tests`
      );
      process.exit(0);
    }
    if (!opts.serverUrl) {
      console.error(import_picocolors2.default.red("Error: --server-url is required when running tests"));
      console.error(import_picocolors2.default.dim("Use --list to list tests without connecting to a server"));
      process.exit(1);
    }
    const options = {
    ...
    console.error(
      import_picocolors2.default.dim(`Running ${summary.totalTests} tests from ${summary.totalSuites} suites...`)
    );
    console.error(import_picocolors2.default.dim(`Connecting to: ${options.serverUrl}
`));
    const result = await runConformanceTests(specs, options);
    const output = formatResults(result, opts.output, opts.verbose);
    if (opts.outputFile) {
      fs2.writeFileSync(opts.outputFile, output, "utf-8");
      console.error(import_picocolors2.default.dim(`Results written to: ${opts.outputFile}`));
    } else {
      console.log(output);
    }
    if (result.totalFailed > 0) {
    ...
    if (opts.verbose && err.stack) {
      console.error(import_picocolors2.default.dim(err.stack));
    }
    process.exit(1);
  }
}
