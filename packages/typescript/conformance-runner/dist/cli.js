...
var packageJsonPath = path2.resolve(__dirname2, "../package.json");
var version = "0.1.0";
try {
...
function findDefaultTestPath() {
  const possiblePaths = [
    path2.resolve(__dirname2, "../../../../test/conformance"),
    path2.resolve(__dirname2, "../../../test/conformance"),
...
async function main() {
  try {
    ...
    if (!validFormats.includes(opts.output)) {
      console.error(pc2.red(`Invalid output format: ${opts.output}`));
      console.error(`Valid formats: ${validFormats.join(", ")}`);
      process.exit(1);
    }
    console.error(pc2.dim(`Loading tests from: ${opts.testPath}`));
    let specs = loadTestSpecs(opts.testPath);
    if (opts.suite && opts.suite.length > 0) {
      specs = filterBySuites(specs, opts.suite);
      if (specs.length === 0) {
        console.error(pc2.red("No matching suites found"));
        process.exit(1);
      }
    }
    if (opts.filter) {
      ...
      if (specs.length === 0) {
        console.error(pc2.red("No tests match the filter pattern"));
        process.exit(1);
      }
    }
    if (opts.list) {
...
${pc2.bold(spec.name)}`);
        for (const test of spec.tests) {
          ...
          if (test.description && opts.verbose) {
            console.log(pc2.dim(`    ${test.description}`));
          }
        }
      }
      console.log("\n" + pc2.dim("\u2500".repeat(50)));
...
Total: ${summary2.totalSuites} suites, ${summary2.totalTests} tests`
    ...
    if (!opts.serverUrl) {
      console.error(pc2.red("Error: --server-url is required when running tests"));
      console.error(pc2.dim("Use --list to list tests without connecting to a server"));
      process.exit(1);
    }
    const options = {
    ...
    console.error(
      pc2.dim(`Running ${summary.totalTests} tests from ${summary.totalSuites} suites...`)
    );
    console.error(pc2.dim(`Connecting to: ${options.serverUrl}
`));
    ...
    if (opts.outputFile) {
      ...
      console.error(pc2.dim(`Results written to: ${opts.outputFile}`));
    } else {
      console.log(output);
    }
    if (result.totalFailed > 0) {
