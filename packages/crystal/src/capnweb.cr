# Cap'n Web RPC Client for Crystal
...
module CapnWeb
  ...
  struct Err(T) < Result(T)
    getter error : CapnWeb::Error

    def initialize(@error : CapnWeb::Error)
    end
      ...
      {% for method in block.body.expressions %}
        {% if method.is_a?(Def) %}
          ...
          def {{ method_name }}(
            ...
            {% end %}
          ) : CapnWeb::Promise({{ return_type }})
            CapnWeb::Promise({{ return_type }}).new(@session, 0_i64, [{{ method_name.stringify }}])
          end
        {% end %}
      {% end %}
    end
