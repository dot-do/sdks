...
/// Represents a value in the RPC system
type RpcValue =
    | Null
    | Bool of bool
    | Number of float
...
module RpcValue =
    /// Parse JSON element to RpcValue
    ...
    /// Convert RpcValue to boxed object
    ...
    /// Convert boxed object to RpcValue
...
// =============================================================================
// Core RPC Operations using Pipe Operator
// =============================================================================
...
/// Uses F#'s pipe operator |> for idiomatic chaining
/// Example: getUsers() |> serverMap (fun u -> getUserProfile(u.id))
let serverMap (f: 'T -> RpcPromise<'U>) (promise: RpcPromise<'T list>) : RpcPromise<'U list> =
    {
        Value = async {
            match! promise.Value with
            | Ok items ->
                // In a real implementation, this sends the map expression to the server
                // for execution in a single round-trip. The server executes f for each
                // item and returns all results at once.
                let! results =
                    items
                    |> List.map (fun item ->
                        async {
...
/// Server-side map for single values
/// Example: getUser(id) |> serverMapSingle (fun u -> getUserProfile(u))
...
/// Bind operation for promise chaining using pipe operator
/// Example: getUser(id) |> bind (fun u -> getProfile(u.profileId))
let bind (f: 'T -> RpcPromise<'U>) (promise: RpcPromise<'T>) : RpcPromise<'U> =
    {
        Value = async {
            match! promise.Value with
            | Ok value ->
...
/// Example: getUsers() |> serverFilter (fun u -> u.active)
let serverFilter (predicate: 'T -> bool) (promise: RpcPromise<'T list>) : RpcPromise<'T list> =
    {
        Value = async {
            match! promise.Value with
            | Ok items ->
                let filtered = items |> List.filter predicate
                return Ok filtered
            | Error e ->
                return Error e
        }
...
/// Take first N items server-side
/// Example: getUsers() |> serverTake 10
let serverTake (count: int) (promise: RpcPromise<'T list>) : RpcPromise<'T list> =
    {
        Value = async {
            match! promise.Value with
            | Ok items ->
                let taken = items |> List.truncate count
                return Ok taken
            | Error e ->
                return Error e
        }
...
/// Skip first N items server-side
/// Example: getUsers() |> serverSkip 10 |> serverTake 10
let serverSkip (count: int) (promise: RpcPromise<'T list>) : RpcPromise<'T list> =
    {
        Value = async {
            match! promise.Value with
            | Ok items ->
                let skipped = items |> List.skip (min count (List.length items))
                return Ok skipped
            | Error e ->
                return Error e
        }
...
/// Create a stub with a capability ID
let withCapability (capId: string) (stub: RpcStub) : RpcStub =
    { stub with CapabilityId = Some capId }

// =============================================================================
// Parallel Execution Combinators
// =============================================================================

/// Execute two promises in parallel and combine results
/// Example: (getUser(id), getProfile(id)) ||> parallel2
let parallel2 (p1: RpcPromise<'T>) (p2: RpcPromise<'U>) : RpcPromise<'T * 'U> =
    {
        Value = async {
...
/// Execute a list of promises in parallel
/// Example: userIds |> List.map getUser |> parallelAll
let parallelAll (promises: RpcPromise<'T> list) : RpcPromise<'T list> =
    {
        Value = async {
            let! results =
                promises
            ...
            let collected =
                ...
                |> Array.toList
                |> List.choose (function Ok v -> Some v | Error _ -> None)
            if List.length collected = List.length promises then
                return Ok collected
            else
                return Error (ServerError ("Some parallel operations failed", None))
        }
        Transform = fun _ -> []
    }
...
type RpcPipelineBuilder() =
    ...
    member _.Zero() : RpcPromise<unit> =
        { Value = async { return Ok () }; Transform = fun _ -> () }
