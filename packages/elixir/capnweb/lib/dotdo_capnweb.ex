defmodule DotDo.CapnWeb do
  @moduledoc """
  DotDo Cap'n Web RPC client for Elixir.
  ...
  ## Basic Usage
      ...
      {:ok, api} = DotDo.CapnWeb.connect("wss://api.example.com")

      # Single round trip for entire chain
      name = api
      |> DotDo.CapnWeb.users()
      |> DotDo.CapnWeb.get(123)
      |> DotDo.CapnWeb.profile()
      |> DotDo.CapnWeb.name()
      |> DotDo.CapnWeb.await!()
  ...
  ## Map Operation (Server-side transformation)
      ...
      squared = api
      |> DotDo.CapnWeb.generateFibonacci(6)
      |> DotDo.CapnWeb.map(fn x -> DotDo.CapnWeb.square(api, x) end)
      |> DotDo.CapnWeb.await!()
      # => [0, 1, 1, 4, 9, 25]

  """

  alias DotDo.CapnWeb.{Session, Stub, Promise, Error}
  ...
  @doc """
  Connects to a DotDo Cap'n Web RPC server.
  ...
  @doc """
  Connects to a DotDo Cap'n Web RPC server, raising on error.
  """
  ...
  ## Example
      ...
      children = [
        {DotDo.CapnWeb.Session, name: MyApp.API, url: "wss://api.example.com"}
      ]

      # Later, anywhere in your app
      api = DotDo.CapnWeb.session(MyApp.API)
      result = api |> DotDo.CapnWeb.users() |> DotDo.CapnWeb.get(123) |> DotDo.CapnWeb.await!()
  ...
  ## Examples
      ...
      {:ok, result} = api |> DotDo.CapnWeb.square(5) |> DotDo.CapnWeb.await()

      case api |> DotDo.CapnWeb.users() |> DotDo.CapnWeb.get(123) |> DotDo.CapnWeb.await() do
        {:ok, user} -> IO.puts(user.name)
        {:error, %DotDo.CapnWeb.Error{type: :not_found}} -> IO.puts("Not found")
      end
  ...
  ## Examples
      ...
      counters = api
      |> DotDo.CapnWeb.generateFibonacci(4)
      |> DotDo.CapnWeb.map(fn x -> DotDo.CapnWeb.makeCounter(api, x) end)
      |> DotDo.CapnWeb.await!()

      # Map over capabilities
      values = counters
      |> DotDo.CapnWeb.map(fn counter -> DotDo.CapnWeb.value(counter) end)
      |> DotDo.CapnWeb.await!()
  ...
  ## Example
      ...
      auth = api |> DotDo.CapnWeb.authenticate(token)

      {:ok, [user_id, permissions, prefs]} = DotDo.CapnWeb.batch([
        auth |> DotDo.CapnWeb.user_id(),
        auth |> DotDo.CapnWeb.permissions(),
        auth |> DotDo.CapnWeb.preferences()
      ])
  ...
  ## Example
      ...
      |> DotDo.CapnWeb.users()
      |> DotDo.CapnWeb.get(123)
      |> DotDo.CapnWeb.rescue(fn _error -> default_user() end)
      |> DotDo.CapnWeb.await!()
  ...
  ## Example
      ...
      defmodule MyHandler do
        use DotDo.CapnWeb.Target

        @impl DotDo.CapnWeb.Target
      ...
      {:ok, handler_ref} = DotDo.CapnWeb.export(api, MyHandler)

      api
      |> DotDo.CapnWeb.notifications()
      |> DotDo.CapnWeb.subscribe(handler_ref)
      |> DotDo.CapnWeb.await!()
...
defmodule DotDo.CapnWeb.Stub do
  @moduledoc """
  ...
  happen when you call `DotDo.CapnWeb.await/1` on a promise.
  """
...
defmodule DotDo.CapnWeb.Promise do
  @moduledoc """
  ...
  @type t :: %__MODULE__{
          ...
          map_fn: (any() -> t()) | nil,
          rescue_fn: (DotDo.CapnWeb.Error.t() -> any()) | nil
        }
  ...
  """
  @spec rescue(t(), (DotDo.CapnWeb.Error.t() -> any())) :: t()
  ...
  """
  @spec await(t(), keyword()) :: {:ok, any()} | {:error, DotDo.CapnWeb.Error.t()}
  def await(%__MODULE__{} = promise, opts \\ []) do
    timeout = Keyword.get(opts, :timeout, 30_000)

    case DotDo.CapnWeb.Session.execute(promise.session, promise, timeout) do
      {:ok, result} ->
        {:ok, result}
  ...
  """
  @spec batch([t()], keyword()) :: {:ok, [any()]} | {:error, DotDo.CapnWeb.Error.t()}
  def batch(promises, opts \\ []) when is_list(promises) do
    timeout = Keyword.get(opts, :timeout, 30_000)
    ...
    results =
      Enum.flat_map(by_session, fn {session, indexed_promises} ->
        ...
        DotDo.CapnWeb.Session.execute_batch(session, promises_only, timeout)
      end)
    ...
    e -> {:error, DotDo.CapnWeb.Error.from_exception(e)}
  end

  # Allow promise chaining via Access protocol
  defimpl Access do
    def fetch(promise, key) do
      {:ok, DotDo.CapnWeb.Promise.property(promise, key)}
    end
...
defmodule DotDo.CapnWeb.Error do
  @moduledoc """
  Error types for DotDo.CapnWeb operations.
  """
...
defmodule DotDo.CapnWeb.Session do
  @moduledoc """
  GenServer managing a WebSocket connection to a DotDo Cap'n Web server.
  ...
  def handle_info(:connect, state) do
    ...
    Logger.debug("DotDo.CapnWeb: Connecting to #{state.url}")
    ...
    case Jason.decode(data) do
      {:ok, %{"id" => id, "result" => result}} ->
        case Map.pop(state.pending, id) do
          {nil, _} ->
            Logger.warning("DotDo.CapnWeb: Received response for unknown request #{id}")
            {:noreply, state}
      ...
      {:ok, %{"id" => id, "error" => error}} ->
        case Map.pop(state.pending, id) do
          ...
          {from, pending} ->
            GenServer.reply(from, {:error, DotDo.CapnWeb.Error.from_rpc_error(error)})
            {:noreply, %{state | pending: pending}}
        end
  ...
  defp build_message(id, %DotDo.CapnWeb.Promise{} = promise) do
    base = %{
      "id" => id,
      "type" => "call",
  ...
  defp send_message(:connected, message) do
    ...
    Logger.debug("DotDo.CapnWeb: Sending #{inspect(message)}")
    :ok
  end

  defp send_message(nil, _message) do
    Logger.warning("DotDo.CapnWeb: Not connected, cannot send message")
    {:error, :not_connected}
  end
end

defmodule DotDo.CapnWeb.Target do
  @moduledoc """
  ...
  defmacro __using__(_opts) do
    quote do
      @behaviour DotDo.CapnWeb.Target
...
defmodule DotDo.CapnWeb.Application do
  @moduledoc false
  ...
  def start(_type, _args) do
    children = [
      ...
      {Registry, keys: :unique, name: DotDo.CapnWeb.Registry}
    ]

    opts = [strategy: :one_for_one, name: DotDo.CapnWeb.Supervisor]
