defmodule CapnWeb.ConformanceTest do
  @moduledoc """
  Conformance tests for the CapnWeb Elixir SDK.
  ...
  alias CapnWeb.Test.ConformanceConfig
  ...
  defp run_conformance_test(test_config) do
    ...
    # Connect to server
    case CapnWeb.connect(server_url) do
      {:ok, api} ->
        try do
          context = %{self: api}
      ...
      {:error, %CapnWeb.Error{type: :not_implemented}} ->
  ...
  defp run_pipeline_test(api, test_config, context) do
    {promises, context} =
      ...
      |> Enum.reduce({%{}, context}, fn step, {promises, ctx} ->
        ...
        promise =
          if args == [] do
            CapnWeb.prop(target, String.to_atom(method))
          else
            CapnWeb.call(target, String.to_atom(method), args)
          end
    ...
    results =
      ...
      |> Enum.map(fn {name, promise} -> {name, CapnWeb.await!(promise)} end)
      |> Map.new()
  ...
  def build_mapper_function(expression, context) do
    ...
    cond do
      ...
      String.match?(expression, ~r/^\w+\s*=>\s*self\.(\w+)\((\w+)\)$/) ->
        ...
        api = context[:self]
        fn x -> CapnWeb.call(api, String.to_atom(method), [x]) end
      ...
      String.match?(expression, ~r/^\w+\s*=>\s*self\.(\w+)\((\w+)\s*\*\s*(\d+)\)$/) ->
        ...
        fn x -> CapnWeb.call(api, String.to_atom(method), [x * mult]) end
      ...
      String.match?(expression, ~r/^(\w+)\s*=>\s*\1\.(\w+)$/) ->
        ...
        fn elem -> CapnWeb.prop(elem, String.to_atom(prop)) end
      ...
      String.match?(expression, ~r/^(\w+)\s*=>\s*\1\.(\w+)\((.+)\)$/) ->
        ...
        fn elem -> CapnWeb.call(elem, String.to_atom(method), args) end
      ...
      true ->
        ...
        Logger.warn("Unknown map expression pattern: #{expression}")
        fn x -> x end
    end
  end
  ...
  defp execute_call(api, call, args, context) do
    ...
    if String.starts_with?(call, "$") do
      ...
      if is_struct(target, CapnWeb.Promise) or is_struct(target, CapnWeb.Stub) do
        if args == [] do
          target
        else
          CapnWeb.call(target, :__identity__, args)
        end
      else
      ...
      case String.split(call, ".") do
        [method] ->
          CapnWeb.call(api, String.to_atom(method), args)
        ...
        [ref_name | method_chain] ->
          ...
          Enum.reduce(method_chain, base, fn method, acc ->
            if args == [] do
              CapnWeb.prop(acc, String.to_atom(method))
            else
              CapnWeb.call(acc, String.to_atom(method), args)
            end
          end)
      end
    end
  ...
  defp assert_type_matches(result, "capability") do
    assert is_struct(result, CapnWeb.Promise) or is_struct(result, CapnWeb.Stub),
  ...
  defp assert_type_matches(result, "array_of_capabilities") do
    ...
    Enum.each(result, fn item ->
      assert is_struct(item, CapnWeb.Promise) or is_struct(item, CapnWeb.Stub),
  ...
  defp run_verifications(result, verifications) do
    Enum.each(verifications, fn verification ->
      ...
      verification_promise = CapnWeb.call(result, String.to_atom(method), [])
      verification_result = CapnWeb.await!(verification_promise)
...
defmodule CapnWeb.SDKTest do
  ...
  describe "CapnWeb.Stub" do
    ...
    test "property/2 extends the path" do
      stub = CapnWeb.Stub.new(self())
      |> CapnWeb.Stub.property(:users)
      |> CapnWeb.Stub.property(:get)
    ...
    test "call/3 returns a Promise" do
      stub = CapnWeb.Stub.new(self())
      promise = CapnWeb.Stub.call(stub, :get, [123])

      assert %CapnWeb.Promise{} = promise
      assert promise.path == [:get]
      assert promise.args == [123]
    end
  end

  describe "CapnWeb.Promise" do
    test "new/3 creates a promise" do
      promise = CapnWeb.Promise.new(self(), [:users, :get], [123])
    ...
    test "property/2 chains operations" do
      promise = CapnWeb.Promise.new(self(), [:users, :get], [123])
      |> CapnWeb.Promise.property(:profile)
      |> CapnWeb.Promise.property(:name)
    ...
    test "call/3 chains method calls" do
      promise = CapnWeb.Promise.new(self(), [:counter], [])
      |> CapnWeb.Promise.call(:increment, [5])
    ...
    test "map/2 stores the mapper function" do
      mapper = fn x -> x * 2 end
      promise = CapnWeb.Promise.new(self(), [:fib], [10])
      |> CapnWeb.Promise.map(mapper)
    ...
    test "rescue/2 stores the error handler" do
      ...
      promise = CapnWeb.Promise.new(self(), [:users, :get], [999])
      |> CapnWeb.Promise.rescue(handler)

      assert promise.rescue_fn == handler
    end
  end

  describe "CapnWeb.Error" do
    test "connection_error/1 creates a connection error" do
      error = CapnWeb.Error.connection_error(:timeout)
    ...
    test "not_implemented/1 creates a not implemented error" do
      error = CapnWeb.Error.not_implemented("websocket")
    ...
    test "from_rpc_error/1 parses RPC error maps" do
      error = CapnWeb.Error.from_rpc_error(%{
        "type" => "not_found",
    ...
    test "implements Exception protocol" do
      error = CapnWeb.Error.connection_error(:refused)
  ...
  describe "CapnWeb pipe operations" do
    test "prop/2 works with stub" do
      stub = CapnWeb.Stub.new(self())
      result = CapnWeb.prop(stub, :users)
    ...
    test "prop/2 works with promise" do
      promise = CapnWeb.Promise.new(self(), [:users, :get], [123])
      result = CapnWeb.prop(promise, :profile)
    ...
    test "call/3 works with stub" do
      stub = CapnWeb.Stub.new(self())
      result = CapnWeb.call(stub, :get, [123])

      assert %CapnWeb.Promise{} = result
      assert result.path == [:get]
    end

    test "call/3 works with promise" do
      promise = CapnWeb.Promise.new(self(), [:counter], [])
      result = CapnWeb.call(promise, :increment, [5])
    ...
    test "map/2 works with promise" do
      promise = CapnWeb.Promise.new(self(), [:fib], [10])
      mapper = fn x -> x * 2 end
      result = CapnWeb.map(promise, mapper)
    ...
    test "rescue/2 works with promise" do
      promise = CapnWeb.Promise.new(self(), [:users, :get], [999])
      handler = fn _e -> :default end
      result = CapnWeb.rescue(promise, handler)
...
defmodule CapnWeb.MapExpressionTest do
  @moduledoc """
