...
# CapnWeb - Capability-based RPC for Ruby
#
# A minimal stub implementation for conformance testing.
# This provides the basic API structure that will be filled in
# as the SDK is implemented.
module CapnWeb
  ...
  class RpcPromise
    ...
    def await(timeout: nil)
      ...
      raise NotImplementedError, 'CapnWeb SDK not yet implemented'
    end
  ...
  class Stub < BasicObject
    ...
    def method_missing(name, *args, **kwargs, &block)
      if args.empty? && kwargs.empty? && block.nil?
        ...
        ::CapnWeb::RpcPromise.new(@session, @path + [name], args, kwargs)
      end
    end
    ...
    def inspect
      "#<CapnWeb::Stub path=#{@path.inspect}>"
    end
